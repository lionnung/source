...
{  Author: Mestdagh Wilfried
   ...
   Version information:
      ...
      1.15 17 apr 2004
         ...
         - New event OnSendProgress. It is fired every time RxBuffer is emtpy.
...
uses
   WinProcs, Windows, Classes, Messages, SysUtils;

const
   CIA_COMMVERSION    = '1.15';
...
type
  ...
  public
    ...
    function  Read(var Data: Pointer; var Len: integer): boolean;
  ...
  TCiaPortOptions = (EventCtsDsr, EventError);
...
begin
   ...
   if not ClassRegistered then begin
      Result := WinProcs.RegisterClass(CIAPORT_WindowClass);
      if Result = 0 then
         Exit;
   end;
...
//------------------------------------------------------------------------------
function TCiaTxBuffer.Read(var Data: Pointer; var Len: integer): boolean;
begin
   Result := False;
...
begin
   ...
   if FOpen and not Value and (FCommThread.FRxCount > 0) then
...
begin
   ...
   if FRxTimeout > 0 then begin
      ...
      if not Assigned(FTimeoutTimer) then begin
         FTimeoutTimer := TCiaTimer.Create; //(nil);
...
//------------------------------------------------------------------------------
//procedure TCiaComPort.TimeoutTimerTimer(Sender: TObject);
procedure TCiaComPort.WMTimer(var Msg: TMessage);
begin
   FTimeoutTimer.Enabled := False;
   if FOpen and (FCommThread.FRxCount > 0) then
      PostMessage(FHandle, WM_DATAAVAILABLE, 0, 0)
   // hier
end;
...
begin
   if not FDestroying then
      if (LineMode or FRxThresholdUse or (FRxTimeout > 0)) and (FCommThread.FRxCount > 0) then
         FOnDataAvailable(Self);
...
begin
   ...
   Len := FTxBuffer;
   if FTxBuf.Read(P, Len) then begin
      InternalSend(P, Len);
      FSending   := True;
      FSendCount := len;
      Exit;
   ...
   if (FEventMask and EV_RXCHAR) > 0 then begin
      ...
      if FInternalRxCount > 0 then
         if not Assigned(FCiaComPort.FOnDataAvailable) then    // nobody wants to receive
            ...
            if FCiaComPort.LineMode or FCiaComPort.RxThresholdUse or (FCiaComPort.FRxTimeout > 0) then
               InternalReceive
            else begin
